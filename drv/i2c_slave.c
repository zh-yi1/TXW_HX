...
/* ---- 判断寄存器地址是否可写 (协议规范 W 区域) ---- */
static uint8_t reg_is_writable(uint8_t addr)
{
    /* 0x00~0x15: 主机信息 + 电池数据前半 (W) */
    if (addr <= 0x15) return 1;
    /* 0x16~0x23: V1~VPACK + BAT_CURRENT + NTC1 (R) */
    if (addr <= 0x23) return 0;
    /* 0x24~0x25: NTC2 (W) */
    if (addr <= 0x25) return 1;
    /* 0x26~0x27: AFE_PROTECT + OVP_PERMANENT (R) */
    if (addr <= 0x27) return 0;
    /* 0x50~0x61: 端口数据 + 保护标志 (W) */
    if (addr >= 0x50 && addr <= 0x61) return 1;
    /* 0x70: KEY_EVENT (R/W) */
    if (addr == 0x70) return 1;
    /* 0xF0~0xF2: 系统信息 (R) */
    if (addr >= 0xF0) return 0;
    /* 其他预留区域: 允许写入 */
    return 1;
}
...
void I2C1_Handler(void)
{
        ...
        && md_i2c_is_enable_it_evt(I2C1))
    {
        ...
        {
            /* 后续字节 = 数据，只写入可写寄存器 */
            if (reg_is_writable(i2c_reg_addr))
            {
...
static void pull_sensor_data(void)
{
    ...
    int16_t rntc = 0;
    if (vntc > 0 && vntc < 2545)
        rntc = (int16_t)(10000UL * vntc / (2545 - vntc));
    i2c_reg_map[REG_NTC1_L] = (uint8_t)(rntc & 0xFF);
    i2c_reg_map[REG_NTC1_H] = (uint8_t)((rntc >> 8) & 0xFF);
