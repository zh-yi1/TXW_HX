...
static void				ui_brightness_draw					(void);

void ui_screen_brightness(void)
{
	switch(ui_ctrl.disp_step)
	{
		case DISP_STEP_REFLASH:
			...
			ui_ctrl.brightness_flag = ui_ctrl.iic_data.sys_status_read.screen_brightness;
			break;
		...
		case DISP_STEP_WAITCMD:
			if(ui_ctrl.brightness_flag != ui_ctrl.iic_data.sys_status_read.screen_brightness)
			{
				ui_ctrl.brightness_flag = ui_ctrl.iic_data.sys_status_read.screen_brightness;
...
static void ui_brightness_draw(void)
{
	...
	uint8_t length_block = length_brightness;

	if(ui_ctrl.iic_data.language_set)
		length_block = length_brightness_cn;
	
	if(ui_ctrl.disp_block < length_block)
	{
		const flash_read_block_t *flash_block = brightness_array + ui_ctrl.disp_block;

		if(ui_ctrl.iic_data.language_set)
			flash_block = brightness_cn_array + ui_ctrl.disp_block;
		...
		if(ui_ctrl.disp_pos >= disp_pos_max)
		{
			if(spi_dma_send_ok)
			{
				...
				if(ui_ctrl.reflash_disp && ui_ctrl.disp_block >= BN_REFLASH_POS)
...
//		if(spi_dma_send_ok || ui_ctrl.disp_pos == 0)
		if(spi_dma_send_ok)
		{
			...
			if(ui_ctrl.disp_block < BN_REFLASH_POS)
			{
				if(ui_ctrl.brightness_flag)
				{
					...
					{
						flash_read_addr_offset = FLASH_ADDR_BN_H_OFFSET;

						if(ui_ctrl.iic_data.language_set)
							flash_read_addr_offset = FLASH_ADDR_CN_BN_H_OFFSET;
					}
				}
				else
				{
					...
					{
						flash_read_addr_offset = FLASH_ADDR_BN_L_OFFSET;

						if(ui_ctrl.iic_data.language_set)
							flash_read_addr_offset = FLASH_ADDR_CN_BN_L_OFFSET;
					}
				}
			}
